...
def count_playlists(client_receiver_id, client_with_playlists_id):
    conn = get_db_connection()
    cur = conn.cursor()

    try:
        sql = """
              SELECT COUNT(outer_pc.playlist_id)
        ...
        playlists_response = cur.fetchone()

        return {
            "count": playlists_response[0],
        }
    except Exception as e:
        print(e.args)
        conn.rollback()
